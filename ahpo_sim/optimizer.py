...
class ProportionalSpreadController:
    ...
    def __init__(
        ...
        self._kp = kp if kp is not None else config.SPREAD_CONTROLLER_KP
        self._deadband_k = deadband_k if deadband_k is not None else config.SPREAD_CONTROLLER_DEADBAND_K
        self._max_step_percent = max_step_percent if max_step_percent is not None else config.SPREAD_CONTROLLER_MAX_STEP_PERCENT
