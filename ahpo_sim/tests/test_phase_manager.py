...
def test_confidence_below_threshold_stays_passive() -> None:
    cm = CharacteristicMap()
    pm = PhaseManager(cm, confidence_threshold=0.7, min_samples=5, max_spread_error_std=1.0)
...
def test_confidence_at_or_above_threshold_becomes_active() -> None:
    cm = CharacteristicMap()
    pm = PhaseManager(cm, confidence_threshold=0.5, min_samples=2, max_spread_error_std=1.0)
    for _ in range(5):
        cell = cm.update(5.0, 40.0, charge_pump_speed=30.0, spread_error=0.1)
    pm.update_confidence(cell)
...
def test_active_cell_fraction_mixed() -> None:
    cm = CharacteristicMap()
    pm = PhaseManager(cm, confidence_threshold=0.5, min_samples=5, max_spread_error_std=1.0)
    for _ in range(5):
        active_cell = cm.update(5.0, 40.0, charge_pump_speed=30.0, spread_error=0.1)
    passive_cell = cm.update(9.0, 44.0, charge_pump_speed=30.0, spread_error=0.1)
...
def test_confidence_no_reference_time_no_decay() -> None:
    cm = CharacteristicMap()
    pm = PhaseManager(cm, confidence_threshold=0.5, min_samples=2, max_spread_error_std=1.0)
    cell = cm.update(5.0, 40.0, charge_pump_speed=30.0, spread_error=0.5, timestamp=pd.Timestamp("2026-01-01"))
...
def test_confidence_age_decay_reduces_score() -> None:
    cm = CharacteristicMap()
    pm = PhaseManager(cm, confidence_threshold=0.5, min_samples=2, max_spread_error_std=1.0, age_halflife_days=30.0)
    cell = cm.update(5.0, 40.0, charge_pump_speed=30.0, spread_error=0.5, timestamp=pd.Timestamp("2026-01-01"))
...
def test_get_phase_drops_to_passive_after_long_staleness() -> None:
    cm = CharacteristicMap()
    pm = PhaseManager(cm, confidence_threshold=0.7, min_samples=1, max_spread_error_std=100.0, age_halflife_days=30.0)
    cell = cm.update(5.0, 40.0, charge_pump_speed=30.0, spread_error=0.1, timestamp=pd.Timestamp("2026-01-01"))
