...
def test_update_creates_cell_and_records_values() -> None:
    ...
    assert cell.best_cop == 4.0
    assert cm.lookup(5.0, 40.0) is cell


def test_update_raises_on_missing_values() -> None:
    # Edge case: missing values (NaN) must fail fast, not silently corrupt the map.
    cm = CharacteristicMap()
...
def test_cop_std_with_single_measurement_is_zero() -> None:
    cm = CharacteristicMap()
    cell = cm.update(5.0, 40.0, charge_pump_speed=30.0, cop=4.0)
    assert cell.cop_std == 0.0


def test_gliding_update_moves_towards_higher_cop_speed() -> None:
    cm = CharacteristicMap()
    cell = cm.update(5.0, 40.0, charge_pump_speed=30.0, cop=4.0)
    cm.update(5.0, 40.0, charge_pump_speed=40.0, cop=8.0)
    assert 30.0 < cell.optimal_charge_pump_speed <= 40.0
    assert cell.best_cop == 8.0


def test_to_dataframe_shape() -> None:
    cm = CharacteristicMap()
    cm.update(5.0, 40.0, charge_pump_speed=30.0, cop=4.0)
    cm.update(9.0, 44.0, charge_pump_speed=35.0, cop=5.0)
    df = cm.to_dataframe()
    assert len(df) == 2
...
def test_to_dict_from_dict_round_trip() -> None:
    cm = CharacteristicMap()
    cm.update(5.0, 40.0, charge_pump_speed=30.0, cop=4.0, timestamp=pd.Timestamp("2026-01-01"))
    records = cm.to_dict()
...
def test_export_json_round_trip(tmp_path) -> None:
    ...
    assert len(loaded) == 1
    assert loaded.loc[0, "best_cop"] == 4.0


def _build_varied_map() -> CharacteristicMap:
    cm = CharacteristicMap()
    cm.update(5.0, 40.0, charge_pump_speed=30.0, cop=4.0, timestamp=pd.Timestamp("2026-01-01"))
    cm.update(5.0, 40.0, charge_pump_speed=32.0, cop=4.5, timestamp=pd.Timestamp("2026-01-02"))
    cm.update(9.0, 44.0, charge_pump_speed=50.0, cop=3.0, timestamp=pd.Timestamp("2026-01-03"))
    return cm
...
def test_import_csv_reconstructs_cell_fields(tmp_path) -> None:
    ...
    assert imported_cell.cop_std == pytest.approx(original_cell.cop_std)
    assert imported_cell.cop_weight_sum == pytest.approx(original_cell.cop_weight_sum)
...
def test_import_csv_round_trip_preserves_phase_decisions(tmp_path) -> None:
    ...
    original_pm = PhaseManager(original, confidence_threshold=0.5, min_samples=2, max_cop_std=1.0)
    ...
    imported = CharacteristicMap()
    imported.import_csv(path)
    imported_pm = PhaseManager(imported, confidence_threshold=0.5, min_samples=2, max_cop_std=1.0)
